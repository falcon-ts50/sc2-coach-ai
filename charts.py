...
def plot_metric(data: dict[str, Any], metric: str, title: str, ylabel: str, path: Path) -> None:
...
def generate_charts(data: dict[str, Any], out: Path) -> list[Path]:
    ...
    for metric, title, ylabel, path in specs:
        plot_metric(data, metric, title, ylabel, path)
        paths.append(path)
    return paths
...
def main() -> int:
    ...
    for path in generate_charts(data, args.out):
        print(path)
    return 0
