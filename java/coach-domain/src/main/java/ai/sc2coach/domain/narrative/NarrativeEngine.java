...
public final class NarrativeEngine {
    ...
    public MatchNarrative build(List<Episode> episodes, List<ArgumentDelta> deltas, String finalLeader) {
        List<Episode> safeEpisodes = Optional.ofNullable(episodes).orElseGet(List::of);
        List<ArgumentDelta> safeDeltas = Optional.ofNullable(deltas).orElseGet(List::of);

        List<MatchNarrative.Beat> beats = safeEpisodes.stream()
                .map(episode -> beat(episode, safeDeltas))
    ...
    private MatchNarrative.Beat beat(Episode episode, List<ArgumentDelta> deltas) {
        List<ArgumentDelta> nearby = nearbyDeltas(episode, deltas);
    ...
    private String statement(Episode episode, List<ArgumentDelta> deltas) {
        return deltas.stream()
                ...
                .orElseGet(() -> episode.title() + ". Эпизод требует проверки по таймлайну и доступным доказательствам.");
    }

    private List<String> evidence(Episode episode, List<ArgumentDelta> deltas) {
        return Stream.concat(
                        episode.evidence().stream(),
                        deltas.stream().map(delta -> delta.playerName() + ": " + component(delta.component())
